...
As mentioned on the [home](https://hockeyr.netlify.app/index.html) page, the main function of the `puckR` package is to load raw NHL play-by-play data without having to scrape it and clean it yourself. The `load_pbp()` function will do that for you. The `season` argument in `load_pbp()` is very accepting. You may use any of the following syntax when loading play-by-play data for the 2020-21 NHL season:
...
This is the backbone function that keeps the [puckR-data repository](https://github.com/zbeg/puckR-data) up to date during the season. Supply a date (`YYY-MM-DD`) and it will scrape play-by-play data for all games on that day. Live game scraping is still awaiting testing.
