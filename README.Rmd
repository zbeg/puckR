...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/puckR)](https://CRAN.R-project.org/package=puckR)
...
Prior seasons still live in the [puckR-data](https://github.com/zbeg/puckR-data) repository and can still be pulled using the `load_pbp` function. The current season will hopefully be updated each night, but is still very new and should be checked during use.

Please be sure to send any and all complaints to gbettman@nhl.com.

------------------------------------------------------------

This package contains various functions to scrape and clean play-by-play data from NHL.com. Season play-by-play data scraped with these functions can be found in the [puckR-data](https://github.com/zbeg/puckR-data) repository. It also contains functions to scrape data from [hockey-reference.com](https://www.hockey-reference.com/), including standings, player stats, and jersey number history.

## Installation

Install the development version of `puckR` from [GitHub](https://github.com/) with:
...
The `shot_distance` and `shot_angle` are measured in feet and degrees, respectively. The variables `x_fixed` and `y_fixed` are transformations of the `x` and `y` event coordinates such that the home team is always shooting to the right and the away team is always shooting to the left. For full details on the included variables, see the [`scrape_game`](https://github.com/zbeg/puckR/blob/master/R/scrape_game.R) documentation.

#### NEW in puckR v1.1.0: Expected Goals

As of `puckR` v1.1.0, a new column has been added to the play-by-play data: Expected goals! The `puckR` package now includes its own public expected goals model, and every unblocked shot in the play-by-play data now has an `xg` value. A full description of the model, including the code used to construct it and the testing results, can be found in the [puckR-models](https://github.com/zbeg/puckR-models) repository. Users can now investigate additional statistics, such as player goals above expectation without having to create their own entire model.
